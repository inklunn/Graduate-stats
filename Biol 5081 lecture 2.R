...
Why do you think this?
  ...
  Count data. Variance and mean unlikely to be the same

No. of taxa identified in individual PCRs vs no. of taxa identified in pooled PCR

The total number of taxa, no. of taxa identified vs. no. which were not -- and then compare between pooled and individual?
  ...
  Using the distribution that you've chosen, change the different parameters. What happens when you change the parameters for the distribution? Describe how the shape changes.
    ...
    Answered within the negative binomial section

Can you simulate data that looks like what you expect your data looks like? What are the parameters for that?
  My data for one target region, e.g. COI
  I don't understand how we are supposed to know what our data is going to look like...
n = number of taxa (genera),  39. Mu, number of genera that are in both pool and individual? Theta, max is sample size? Can it be bigger? I     think i'm confusing myself

  hist(MASS::rnegbin(39, mu=8, theta=39))
...
hist(MASS::rnegbin(400, mu=5, theta=1000))
# Changing the n, changes sample size so overall records of frequency change
# Changing the mu, changes where the largest bar is, changes the mean
# Changing theta, not sure? Doesn't seem to impact the chart in a consistent way?
  
  # Make a plot of the density of NBV simulation - not going to lie, I don't really understand that this is showing
  plot(density(MASS::rnegbin(100, mu=5, theta=10)))
# Increasing the n, seems to skew the graph right slightly?
# Changing the mu, both density and and bandwidth increase with mu
# Changing theta, again not sure?
